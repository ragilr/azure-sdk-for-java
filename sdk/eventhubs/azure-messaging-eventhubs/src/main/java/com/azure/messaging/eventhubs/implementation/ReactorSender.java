...
class ReactorSender extends EndpointStateNotifierBase implements AmqpSendLink {
    ...
    public Mono<Void> send(Message message) {
        ...
        final int allocationSize = Math.min(payloadSize + ClientConstants.MAX_EVENTHUB_AMQP_HEADER_SIZE_BYTES,
            maxMessageSize);
        ...
        try {
            ...
            final String errorMessage = String.format(Locale.US, "Error sending. Size of the payload exceeded maximum"
                + " message size: %s kb", maxMessageSize / 1024);
    ...
    public Mono<Void> send(List<Message> messageBatch) {
        ...
        for (final Message amqpMessage : messageBatch) {
            ...
            try {
                encodedSize = messageWrappedByData.encode(bytes, byteArrayOffset,
                    maxMessageSizeTemp - byteArrayOffset - 1);
            } catch (BufferOverflowException exception) {
                final String message = String.format(Locale.US, "Size of the payload exceeded maximum message size: "
                    + "%s kb", maxMessageSizeTemp / 1024);
    ...
    private void processSendWork() {
        ...
        while (hasConnected.get() && sender.getCredit() > 0) {
            ...
            if (workItem == null) {
                if (deliveryTag != null) {
                    logger.verbose("clientId[{}]. path[{}], linkName[{}], deliveryTag[{}]: sendData not found for "
                            + "this delivery.",
