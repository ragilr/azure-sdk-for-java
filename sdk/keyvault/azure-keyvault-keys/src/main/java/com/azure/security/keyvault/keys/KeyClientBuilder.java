 ...
 * secret async client} and {@link KeyClient secret sync client}, by calling {@link KeyClientBuilder#buildAsyncClient()
 * buildAsyncClient} and {@link KeyClientBuilder#buildClient() buildClient} respectively It constructs an instance of
 * the desired client.
 *
...
public final class KeyClientBuilder {
     ...
     * key vault endpoint} are required to build the {@link KeyClient client}.</p>
     *
     * @return A {@link KeyClient} with the options set from the builder.
     *
     * @throws IllegalStateException If {@link KeyClientBuilder#credential(TokenCredential)} or {@link
     * KeyClientBuilder#endpoint(String)} have not been set.
     */
    ...
    /**
     * Creates a {@link KeyAsyncClient} based on options set in the builder. Every time {@code buildAsyncClient()} is
     * called, a new instance of {@link KeyAsyncClient} is created.
     *
     * <p>If {@link KeyClientBuilder#pipeline(HttpPipeline) pipeline} is set, then the {@code pipeline} and
     * {@link KeyClientBuilder#endpoint(String) serviceEndpoint} are used to create the {@link KeyClientBuilder client}.
     * All other builder settings are ignored. If {@code pipeline} is not set, then {@link
    ...
    public KeyAsyncClient buildAsyncClient() {
        ...
        policies.add(
            new UserAgentPolicy(
                AzureKeyVaultConfiguration.SDK_NAME, AzureKeyVaultConfiguration.SDK_VERSION, buildConfiguration));
     ...
     *
     * If {@code pipeline} is set, all other settings are ignored, aside from {@link KeyClientBuilder#endpoint(String)
     * endpoint} to build {@link KeyClient} or {@link KeyAsyncClient}.
     *
