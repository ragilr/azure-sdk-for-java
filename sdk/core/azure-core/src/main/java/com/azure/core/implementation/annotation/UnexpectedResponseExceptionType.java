 ...
 * annotations can be used. When no codes are listed that exception is always thrown or returned if it is reached during
 * evaluation, this should be treated as a default case. If no default case is annotated the fall through exception is
 ...
 * </pre>
 *
 * If multiple annotations share the same HTTP status code or there is multiple default annotations the exception from
 * the last annotation in the top to bottom order will be used.
 */
@Retention(RUNTIME)
@Target(METHOD)
...
public @interface UnexpectedResponseExceptionType {
    /**
     * The type of HttpResponseException that should be thrown/returned when the API returns an unrecognized status
     * code.
     *
