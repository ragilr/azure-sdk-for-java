...
final class HttpResponseHeaderDecoder {
        ...
        if (headerType == null) {
            ...
            return Mono.defer(() -> {
                try {
                    ...
                    return Mono.error(new HttpResponseException("HTTP response has malformed headers", httpResponse,
                        e));
                }
            });
        }
     ...
     * 'Matching' headers.
     *
     * 'Matching' headers are the REST API returned headers those with: 1. header names same as name of a properties in
     * the entity. 2. header names start with value of {@link HeaderCollection} annotation applied to the properties in
     * the entity.
     *
     * When needed, the 'header entity' types must be declared as first generic argument of {@link ResponseBase}
     * returned by java proxy method corresponding to the REST API. e.g. {@code Mono<RestResponseBase
     * <FooMetadataHeaders, Void>> getMetadata(args);} {@code class FooMetadataHeaders { String name;
     *
     * @param headers the REST API returned headers
     *
     * @return instance of header entity type created based on provided {@headers}, if header entity model does not
    ...
    private static Object deserializeHeaders(HttpHeaders headers, SerializerAdapter serializer,
        ...
        if (deserializedHeadersType == null) {
            ...
            Object deserializedHeaders = serializer.deserialize(headersJsonString, deserializedHeadersType,
