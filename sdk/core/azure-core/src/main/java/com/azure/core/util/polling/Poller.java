...
/**
 * This class offers API that simplifies the task of executing long-running operations against Azure service. The {@link
 ...
 * For those scenarios which require manual control of the polling cycle, disable auto-poling by calling {@code
 * setAutoPollingEnabled#false} and perform manual poll
...
public class Poller<T> {
     ...
     * the client library, which must never return {@code null}, and which must always have a non-null {@link
     * OperationStatus}. {@link Mono} returned from poll operation should never return {@link Mono#error(Throwable)}.If
     * any unexpected scenario happens in poll operation, it should be handled by client library and return a valid
     ...
     * the client library, which must never return {@code null}, and which must always have a non-null {@link
     * OperationStatus}. {@link Mono} returned from poll operation should never return {@link Mono#error(Throwable)}.If
     * any unexpected scenario happens in poll operation, it should handle it and return a valid {@link PollResponse}.
     * However if poll operation returns {@link Mono#error(Throwable)}, the {@link Poller} will disregard that and
     * continue to poll.
    ...
    public void cancelOperation() throws UnsupportedOperationException {
        if (this.cancelOperation == null) {
            throw logger.logExceptionAsError(
                new UnsupportedOperationException("Cancel operation is not supported on this service/resource."));
        }
    ...
    /**
     * Enable user to take control of polling and trigger manual poll operation. It will call poll operation once. This
     ...
     * @param statusToBlockFor The desired {@link OperationStatus} to block for and it can be any valid {@link
     * OperationStatus} value.
     * @param timeout The time after which it will stop blocking. A {@code null} value will cause to block indefinitely.
     * Zero or negative are not valid values.
     *
    ...
    public PollResponse<T> blockUntil(OperationStatus statusToBlockFor, Duration timeout) {
        if (statusToBlockFor == null) {
            throw logger.logExceptionAsWarning(
                new IllegalArgumentException("Null value for status is not allowed."));
        }
