 ...
 * information such as the current {@link OperationStatus status} of the long-running operation, any {@link #getValue
...
public final class PollResponse<T> {
    ...
    /**
     * An enum to represent all possible states that a long-running operation may find itself in. The poll operation is
     * considered complete when the status is one of {@code SUCCESSFULLY_COMPLETED}, {@code USER_CANCELLED} or {@code
     * FAILED}.
     */
    public static final class OperationStatus extends ExpandableStringEnum<OperationStatus> {
        ...
        /**
         * Represents that this long-running operation is cancelled by user, however this is still considered as
     ...
     * {@code null}, zero or negative value will be taken to mean that the {@link Poller} should determine on its own
     * when the next poll operation is to occur.
    ...
    /**
     * Represents the status of the long-running operation at the time the last polling operation finished
     * successfully.
     *
