...
public class RestProxy implements InvocationHandler {
    ...
    public Object invoke(Object proxy, final Method method, Object[] args) {
        try {
            ...
            if (method.isAnnotationPresent(ResumeOperation.class)) {
                ...
                return handleResumeOperation(request, opDesc, methodParser, returnType, startTracingSpan(resumeMethod,
                    Context.NONE));

            } else {
                methodParser = methodParser(method);
                request = createHttpRequest(methodParser, args);
                Context context = methodParser.context(args).addData("caller-method",
    ...
    private HttpRequest createHttpRequest(OperationDescription operationDescription, SwaggerMethodParser methodParser,
        ...
        final HttpRequest request = configRequest(new HttpRequest(methodParser.httpMethod(),
            operationDescription.url()), methodParser, args);
        ...
        if (bodyContentObject == null) {
            ...
            if (isJson) {
                ...
                final String bodyContentString = serializer.serialize(bodyContentObject,
                    SerializerEncoding.fromHeaders(request.headers()));
                request.body(bodyContentString);
            }
        }
     ...
     * the HTTP request.
     * @param additionalAllowedStatusCodes Additional allowed status codes that are permitted based on the context of
     * the HTTP request.
     *
            ...
            asyncResult = bodyAsString.flatMap((Function<String, Mono<HttpDecodedResponse>>) responseContent -> {
                ...
                return decodedErrorBody
                    .flatMap((Function<Object, Mono<HttpDecodedResponse>>) responseDecodedErrorObject -> {
                        // decodedBody() emits 'responseDecodedErrorObject' the successfully decoded exception body
                        // object
                        Throwable exception =
    ...
    private Response<?> createResponse(HttpDecodedResponse response, Type entityType, Object bodyAsObject) {
        ...
        if (cls.equals(Response.class)) {
            ...
            if (bodyAsObject != null && !TypeUtil.isTypeOrSubTypeOf(bodyAsObject.getClass(), Page.class)) {
                throw logger.logExceptionAsError(new RuntimeException("Unable to create PagedResponse<T>. Body must "
                    + "be of a type that implements: " + Page.class));
            }
        }
        ...
        for (Constructor<?> constructor : constructors) {
            ...
            try {
                ...
                switch (paramCount) {
                    ...
                    default:
                        throw logger.logExceptionAsError(new IllegalStateException("Response constructor with "
    ...
    protected final Mono<?> handleBodyReturnType(final HttpDecodedResponse response,
        ...
        if (httpMethod == HttpMethod.HEAD
            && (TypeUtil.isTypeOrSubTypeOf(entityType, Boolean.TYPE) || TypeUtil.isTypeOrSubTypeOf(entityType,
            Boolean.class))) {
    ...
    public final Object handleRestReturnType(Mono<HttpDecodedResponse> asyncHttpDecodedResponse,
                                             ...
                                             Context context) {
        final Mono<HttpDecodedResponse> asyncExpectedResponse = ensureExpectedStatus(asyncHttpDecodedResponse,
            methodParser)
