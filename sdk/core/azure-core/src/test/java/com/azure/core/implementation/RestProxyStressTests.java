...
public class RestProxyStressTests {
    ...
    private static void launchTestServer() throws IOException {
        ...
        if (portString != null) {
            ...
            LoggerFactory.getLogger(RestProxyStressTests.class).warn("Attempting to connect to already-running test "
                + "server on port {}", port);
        } else {
    ...
    private static final class ThrottlingRetryPolicy implements HttpPipelinePolicy {
        ...
        Mono<HttpResponse> process(final int waitTimeSeconds, final HttpPipelineCallContext context,
                                   final HttpPipelineNextPolicy nextPolicy) {
            return nextPolicy.clone().process().flatMap(httpResponse -> {
                if (httpResponse.statusCode() != 503 && httpResponse.statusCode() != 500) {
                    return Mono.just(httpResponse);
                } else {
                    LoggerFactory.getLogger(getClass()).warn("Received " + httpResponse.statusCode() + " for request."
                        + " Waiting " + waitTimeSeconds + " seconds before retry.");
                ...
                if (throwable instanceof IOException) {
                    ...
                    return process(context, nextPolicy).delaySubscription(Duration.of(waitTimeSeconds,
                        ChronoUnit.SECONDS));
                }
    ...
    interface IOService {
        ...
        Mono<VoidResponse> upload100MB(@PathParam("id") String id,
                                       @PathParam(value = "sas", encoded = true) String sas, @HeaderParam("x-ms-blob"
            + "-type") String blobType, @BodyParam(ContentType.APPLICATION_OCTET_STREAM) Flux<ByteBuffer> stream,
                                       @HeaderParam("content-length") long contentLength);

        @Get("/javasdktest/upload/100m-{id}.dat?{sas}")
        Mono<StreamResponse> download100M(@PathParam("id") String id,
                                          @PathParam(value = "sas", encoded = true) String sas);

        @ExpectedResponses({201})
        @Put("/testcontainer{id}?restype=container&{sas}")
        Mono<VoidResponse> createContainer(@PathParam("id") String id,
                                           @PathParam(value = "sas", encoded = true) String sas);

        @ExpectedResponses({202})
        @Delete("/testcontainer{id}?restype=container&{sas}")
        Mono<VoidResponse> deleteContainer(@PathParam("id") String id,
                                           @PathParam(value = "sas", encoded = true) String sas);
    }
    ...
    private static void create100MFiles(boolean recreate) throws IOException {
        ...
        //            LoggerFactory.getLogger(RestProxyStressTests.class).info("Temp files directory already exists:
        //            " + tempFolderPath.toAbsolutePath());
        //        } else {
        //            LoggerFactory.getLogger(RestProxyStressTests.class).info("Generating temp files in directory: "
        //            + tempFolderPath.toAbsolutePath());
        ...
        //                    final AsynchronousFileChannel file = AsynchronousFileChannel.open(filePath,
        ...
        //                        try {
        //                            file.close();
        //                            Files.write(tempFolderPath.resolve("100m-" + i + "-md5.dat"), messageDigest
        //                            .digest());
        //                            LoggerFactory.getLogger(RestProxyStressTests.class).info("Finished writing file
        //                            " + i);
        //                        } catch (Exception e) {
    ...
    public void upload100MParallelTest() {
        //        final String sas = System.getenv("JAVA_SDK_TEST_SAS") == null ? "" : System.getenv
        //        ("JAVA_SDK_TEST_SAS");
        //
        ...
        //                    try {
        //                        fileStream = AsynchronousFileChannel.open(tempFolderPath.resolve("100m-" + id + "
        //                        .dat"));
        //                    } catch (IOException ioe) {
        //                        Exceptions.propagate(ioe);
        //                    }
        //                    return service.upload100MB(String.valueOf(id), sas, "BlockBlob", FluxUtil
        //                    .byteBufStreamFromFile(fileStream), FILE_SIZE).map(response -> {
    ...
    public void uploadMemoryMappedTest() {
        ...
        //                    try {
        //                        fileStream = FileChannel.open(tempFolderPath.resolve("100m-" + id + ".dat"),
        ...
        //                    try {
        //                        MappedByteBuffer mappedByteBufferFile = fileStream.map(FileChannel.MapMode
        //                        .READ_ONLY, 0, fileStream.size());
        ...
        //                    }
        //                    //
        //                    return service.upload100MB(String.valueOf(id), sas, "BlockBlob", stream, FILE_SIZE).map
        //                    (response -> {
        //                        String base64MD5 = response.headers().value("Content-MD5");
        //                        byte[] receivedMD5 = Base64.getDecoder().decode(base64MD5);
        //                        Assert.assertArrayEquals(md5, receivedMD5);
                    ...
                    try {
                        ...
                        return content.last().doOnSuccess(b -> {
                            ...
                            LoggerFactory.getLogger(getClass())
                                .info("Finished downloading and MD5 validated for " + id);
        ...
        Flux.range(0, NUM_FILES)
            .zipWith(md5s, (integer, md5) -> {
                ...
                Flux<ByteBuffer> downloadContent = service.download100M(String.valueOf(id), sas)
                    // Ideally we would intercept this content to load an MD5 to check consistency between download
                    // and upload directly,
                    // but it's sufficient to demonstrate that no corruption occurred between
                    // preparation->upload->download->upload.
                    .flatMapMany(StreamResponse::value)
                    .map(reactorNettybb -> {
                        //
                        // This test 'downloadUploadStreamingTest' exercises piping scenario.
                        //
                        //   A. Receive ByteBufFlux from reactor-netty from NettyInbound.receive() [via service
                        //   .download100M].
                        //   B. Directly pass this ByteBufFlux to Outbound.send() [via service.upload100MB]
                        //
                        //   NettyOutbound.send(NettyInbound.receive())
                        //
                        // A property of ByteBufFlux publisher is - The chunks in the stream gets released
                        // automatically once 'onNext' returns.
                        //
                        // The Outbound.send method subscribe to ByteBufFlux
                        //     1. on each onNext call, the received ByteBuf chunk gets 'scheduled' to write through
                        //     Netty.write()
                        //     2. onNext returns.
                        //     3. repeat 1 & 2 until stream completes or errored.
                        //
                        // The scheduling & immediate return from onNext [1 & 2] can result in the a chunk of
                        // ByteBufFlux to be released
                        ...
                        //   b. Netty.write() calls release on the ByteBuf after write is done. We have double
                        //   release problem here.
                        //
                        // Solution is to aware of ByteBufFlux auto-release property and retain each chunk before
                        // passing to Netty.write().
                        //
                ...
                innerService.createContainer(integer.toString(), sas)
                    .onErrorResume(throwable -> {
                        if (throwable instanceof HttpResponseException) {
                            ...
                            if ((restException.response().statusCode() == 409
                                || restException.response().statusCode() == 404)) {
                                return Mono.empty();
                            }
                        }
                        return Mono.error(throwable);
