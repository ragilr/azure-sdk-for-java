...
public final class AzureProxy extends RestProxy {
    ...
    private static String getDefaultUserAgentString(Class<?> swaggerInterface) {
        final String packageImplementationVersion = swaggerInterface == null
            ? ""
            : "/" + swaggerInterface.getPackage().getImplementationVersion();
    ...
    public static <A> A create(Class<A> swaggerInterface, AzureEnvironment azureEnvironment,
        ...
        final SwaggerInterfaceParser interfaceParser = new SwaggerInterfaceParser(swaggerInterface,
            createDefaultSerializer(), baseUrl);
    ...
    protected Object handleHttpResponse(final HttpRequest httpRequest, Mono<HttpDecodedResponse> asyncHttpResponse,
        ...
        if (TypeUtil.isTypeOrSubTypeOf(returnType, Flux.class)) {
                ...
                return asyncHttpResponse.flatMapMany(httpResponse -> {
                    return createPollStrategy(httpRequest, Mono.just(httpResponse), methodParser)
                        .flatMapMany(pollStrategy -> {
                            Mono<OperationStatus<Object>> first = handleBodyReturnType(httpResponse, methodParser,
                                operationStatusResultType)
                                .map(operationResult -> new OperationStatus<Object>(operationResult,
                                    pollStrategy.status()))
                                .switchIfEmpty(Mono.defer((Supplier<Mono<OperationStatus<Object>>>) () -> Mono
                                    .just(new OperationStatus<>((Object) null, pollStrategy.status()))));
            ...
            final Mono<HttpResponse> lastAsyncHttpResponse = createPollStrategy(httpRequest, asyncHttpResponse,
                methodParser)
            ...
            .flatMap((Function<HttpDecodedResponse, Mono<PollStrategy>>) originalHttpDecodedResponse -> {
                ...
                return ensureExpectedStatus(originalHttpDecodedResponse, methodParser, longRunningOperationStatusCodes)
                    .flatMap(response -> {
                        ...
                        final long delayInMilliseconds = parsedDelayInMilliseconds != null ? parsedDelayInMilliseconds
                        ...
                        if (httpStatusCode == 200) {
                            pollStrategy = AzureAsyncOperationPollStrategy.tryToCreate(AzureProxy.this,
                                methodParser, originalHttpRequest, originalHttpResponse, delayInMilliseconds);
                            if (pollStrategy != null) {
                                result = Mono.just(pollStrategy);
                            } else {
                                result = createProvisioningStateOrCompletedPollStrategy(originalHttpRequest,
                            ...
                            if (httpStatusCode == 201) {
                                ...
                                pollStrategy = AzureAsyncOperationPollStrategy.tryToCreate(AzureProxy.this,
                                    methodParser, originalHttpRequest, originalHttpResponse, delayInMilliseconds);
                                if (pollStrategy == null) {
                                    pollStrategy = LocationPollStrategy.tryToCreate(AzureProxy.this, methodParser,
